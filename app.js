...
async function createTeam() {
    ...
    while (role != 'All Done') {
        ...
        switch (role) {
            ...
            newTeamMember = new Manager(employeeInfo.name,employeeInfo.email,employeeInfo.id,specialInfo.officeNumber)
            break
            
            case 'Engineer':
            specialInfo = await inquirer.prompt(engineerQuestions)
            newTeamMember = new Engineer(employeeInfo.name,employeeInfo.email,employeeInfo.id,specialInfo.github)
            break
            
            case 'Intern':
            specialInfo = await inquirer.prompt(internQuestions)
            newTeamMember = new Intern(employeeInfo.name,employeeInfo.email,employeeInfo.id,specialInfo.school)
            break
